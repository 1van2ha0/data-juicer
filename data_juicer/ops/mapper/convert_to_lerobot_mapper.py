...
class ConvertToLeRobotMapper(Mapper):
    ...
    def _load_config(self, config_path):
        ...
        if "dataset" in external_config:
            ...
            for field_name in ["robot_type", "fps", "chunks_size", "video_dir", "hdf5_filename", "data_root"]:
    ...
    def _convert_trajectory_to_df(
        ...
        state_key = lerobot_keys.get("state", "observation.state")
        action_key = lerobot_keys.get("action", "action")
        annotation_keys = lerobot_keys.get("annotation", ["annotation.human.action.task_description", "annotation.human.action.valid"])

        # 1. Get state and action
        for key_type in ["state", "action"]:
            if key_type not in mapping: continue
    ...
    def _convert_file(self, input_file: str, output_dir: str):
        ...
        # Determine data_root and video_dir
        # If data_root is not in config, assume it's the parent of input_file
        if "data_root" not in config:
            config["data_root"] = input_path.parent
        else:
            config["data_root"] = Path(config["data_root"])
            
        # Resolve video_dir
        if "video_dir" in config:
            video_dir = config["data_root"] / config["video_dir"]
        else:
        ...
        trajectory_ids = sorted([k for k in hdf5_data.keys() if k.startswith("demo_")], key=lambda x: int(x.split("_")[1]))
            ...
            for lerobot_key, view_suffix in video_mapping.items():
                try:
                    ...
                    if original_video_path.exists():
                        ...
                        # Try checking if video_dir is relative to data_root if we guessed wrong
                        pass
    ...
    def process_batched(self, samples: Dict[str, Any], rank: Optional[int] = None) -> Dict[str, Any]:
        ...
        for input_file, output_dir in zip(input_files, output_dirs):
            try:
                self._convert_file(input_file, output_dir)
            except Exception as e:
